...
const EXERCISES = [
  ...
  { id: "widepressup", name: "Wide Press-up", loc: ["home", "outdoor", "gym"], unit: "bw", w: 0, bw: true, credits: { chest: 1, triceps: 0.25, frontdelts: 0.5 }, note: "Wide hands bias the pecs; easy on the elbows, keep shoulders packed." },
  { id: "widepressupincline", name: "Wide Press-up (incline)", loc: ["home", "outdoor", "gym"], unit: "bw", w: 0, bw: true, credits: { chest: 1, triceps: 0.25, frontdelts: 0.5 }, note: "Hands elevated — easier regression of the wide press-up, same chest bias." },
...
/* ---------- Seed (first run): empty state — each device keeps its own log ---------- */
function seedState() {
  return {
    version: 1,
    sessions: [],
    ...
    working: {},
    lastReps: {},
    deloadAnchor: startOfWeekISO(todayISO()),
    deloadWeeks: [],
    lastLocation: "home",
    historyMerged: true,
  };
    ...
    <div style={{ background: C.bg, minHeight: "100vh", color: C.text, fontFamily: fontStack }}>
      ...
      {settingsOpen && (
        <SettingsSheet
          ...
          onReset={() => { const seed = seedState(); setState(seed); setDraft(null); setToast("All data wiped — starting fresh."); }}
        />
      )}
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-end" style={{ background: "rgba(0,0,0,.55)" }} onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} className="w-full rounded-t-2xl" style={{ background: C.surface, borderTop: `1px solid ${C.borderLite}`, maxWidth: 480, margin: "0 auto", padding: "16px 16px 24px", maxHeight: "85vh", overflowY: "auto" }}>
        ...
        {confirmReset ? (
          <button onClick={onReset} className="w-full rounded-xl" style={{ background: C.red, color: "#1A0B0B", padding: "12px 14px", fontSize: 13.5, fontWeight: 600, textAlign: "left" }}>Confirm — wipe all data &amp; start fresh</button>
        ) : (
        ...
        <div style={{ fontSize: 10.5, color: C.faint, marginTop: 14, lineHeight: 1.5 }}>
          Data lives in this browser on this device and persists between visits. Nothing syncs or leaves the device — anyone else using the app link keeps their own separate log. Use Backup/Restore to move yours between devices.
        </div>
      </div>
    </div>
  );
